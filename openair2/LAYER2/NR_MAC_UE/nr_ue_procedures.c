  ...
  if(dlsch_config_pdu != NULL) {
    if (pdsch_Config &&
      ...
      for (int i = 1; i < n_RBG - 1; i++) {
        // The order of RBG bitmap is such that RBG 0 to RBG n_RBG 竏? 1 are mapped from MSB to LSB
...
static int find_pucch_resource_set(NR_PUCCH_Config_t *pucch_Config, int size)
{
  ...
  // a first set of PUCCH resources with pucch-ResourceSetId = 0 if O UCI 竕? 2 including 1 or 2 HARQ-ACK information bits
  if (size <= 2)
    return 0;
  ...
  // a second set of PUCCH resources with pucch-ResourceSetId = 1, if provided by higher layers, if 2 < O UCI 竕? N 2
  if (size <= N2)
    return 1;
  // a third set of PUCCH resources with pucch-ResourceSetId = 2, if provided by higher layers, if N 2 < O UCI 竕? N 3
  if (size <= N3)
    return 2;
  // a fourth set of PUCCH resources with pucch-ResourceSetId = 3, if provided by higher layers, if N 3 < O UCI 竕? 1706
  return 3;
}
...
static void extract_00_c_rnti(dci_pdu_rel15_t *dci_pdu_rel15, const uint8_t *dci_pdu, int pos)
{
  ...
  // Time domain assignment 4bit
  EXTRACT_DCI_ITEM(dci_pdu_rel15->time_domain_assignment.val, 4);
  // Frequency hopping flag 窶・1 bit
  ...
  // TPC command for scheduled PUSCH 窶・2 bits
  EXTRACT_DCI_ITEM(dci_pdu_rel15->tpc, 2);
  // UL/SUL indicator 窶・1 bit
