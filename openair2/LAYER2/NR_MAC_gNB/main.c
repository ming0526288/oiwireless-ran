...
size_t dump_mac_stats(gNB_MAC_INST *gNB, char *output, size_t strlen, bool reset_rsrp)
{
  ...
  uint64_t  dl_bytes_old = 0;
  uint64_t  ul_bytes_old = 0;
  ...
  UE_iterator(gNB->UE_info.list, UE) {
    ...
    dl_bytes_old =  stats->dl.total_bytes_old;
    ul_bytes_old =  stats->ul.total_bytes_old;
  ...
  output += snprintf(output,
    ...
    if (reset_rsrp) {
      stats->dl.total_bytes_old = dl_bytes_new;   
      stats->ul.total_bytes_old = ul_bytes_new;    
